    ...
    this.fdb.list("/users").valueChanges().subscribe((data) => {
      data.forEach((each) => {
        this.users[each['userId']] = each;
           ...
           if(this.users[uid])
             this.currentfullName=this.users[ uid ]['username']
     if((this.users[ uid ])&&(this.users)&&(this.users[ uid ]['fullname'])) {
           console.log(this.users[ uid ]['pictureUrl']);
        this.currentpictureUrl = this.users[ uid ]['pictureUrl'];
  ...
  addUser(uid,_user) {
    this.fdb.list("/users").set(uid, _user).then( (response) => {
      console.log(response);
        this.fdb.list("/users").update(uid,{userId : uid});
    }).catch(err => {
        alert(err);
    });
  }
